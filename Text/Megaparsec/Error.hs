...
data ParseError t e
  = TrivialError Int (Maybe (ErrorItem t)) (Set (ErrorItem t))
...
errorOffset :: ParseError t e -> Int
  ...
  where
    ...
    f (o, !pstate) e = (o . (outChunk ++), pstate')
      where
        ...
        eline = sourceLine epos
        ecol  = sourceColumn epos
        outChunk =
...
reachOffset
  ...
  => Int               -- ^ Offset
  -> PosState s        -- ^ Starting positional state
  -> (SourcePos, Tokens s, PosState s)
     -- ^ Position of the given offset, the relevant line, and the updated
     -- positional state
reachOffset = undefined -- TODO Will need scan for this.
...
----------------------------------------------------------------------------
-- FIXME Position advancing helpers (may be of use yet)
